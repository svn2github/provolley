...
public class CoupeHelper {
		...
		try {
			...
			for (int i = 0; i < journees.length(); i++) {
				...
				for (int j = 0; j < matchs.length(); j++) {
					...
					ResultatsMatch resultatsMatch = new ResultatsMatch(saison, competition, codeMatch, codeDomicile, equipeDomicile, classementDomicile,
							codeExterieur, equipeExterieur, classementExterieur, resultat, score, victoire);
	...
	public static boolean isVictoireDomicile(ResultatsMatch match) {
		if (ProVolley.RESULTAT_VICTOIRE_DOMICILE.equals(match.getVictoire()))
			return true;
		return false;
	}

	public static boolean isVictoireExterieur(ResultatsMatch match) {
		if (ProVolley.RESULTAT_VICTOIRE_EXTERIEUR.equals(match.getVictoire()))
			return true;
		return false;
	}
